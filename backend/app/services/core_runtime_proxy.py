...
*Knowledge / RAG*: same shape. ``app.services.embedding``, ``vector_store``,
``chunker``, ``file_parser`` and ``reranker`` live only in core, and only
core is configured with a vector store and an embedding provider. Studio's
knowledge write/search endpoints therefore forward to core's identical
``/api/v1/knowledge`` endpoints.

So these endpoints forward to core's identical routes under the same prefix
and return core's response verbatim (identical response schemas — the
routers are siblings of the same original file). Core writes the health
...
# Embedding a large document is one provider round-trip per chunk, which runs
# well past the MCP-sized default. Knowledge writes pass this instead.
...
async def _relay(
    ...
    *,
    ...
    ``CORE_API_URL`` — the fix is almost always "start goku-core" or "point
    CORE_API_URL at it".

    Every verb goes through here: the per-verb helpers below used to carry
    their own copy of this block, and each new one (get_from_core was the
    third) reintroduced the same 25 lines.
    """
...
async def get_from_core(
    ...
    *,
    ...
    """GET ``path`` from goku-core. Same relay semantics as :func:`post_to_core`.

    Used for reads that must reflect core's own state — e.g. skill files, which
    the executor loads from core's skills root; Studio's local checkout is a
    different, possibly stale copy.
    """
    resp = await _relay(request, "GET", path, timeout=timeout, purpose=purpose)
    return resp.json()
...
async def put_to_core(
    ...
    *,
    ...
    Used for writes that must run in core — e.g. a capability ``result-script``
    (the sandbox lives in core).
    """
...
async def post_file_to_core(
    ...
    *,
    ...
    ``form_fields`` entries whose value is ``None`` are dropped so core sees an
    absent field rather than the string ``"None"``.
    """
