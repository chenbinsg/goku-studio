...
async def get_from_core(request: Request, path: str) -> dict:
    ...
    502 when core is unreachable). Used for reads that must reflect core's own
    state — e.g. skill files, which the executor loads from core's skills root;
    Studio's local checkout is a different, possibly stale copy."""
    url = settings.CORE_API_URL.rstrip("/") + path
    headers = {}
    authorization = request.headers.get("authorization")
    if authorization:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                f"无法连接 goku-core runtime（{settings.CORE_API_URL}）：{exc}。"
                f"请确认 goku-core 服务已启动，或通过环境变量 CORE_API_URL 指向正确地址。"
            ),
        ) from exc
